...
# 🔥 FIRE SCENARIO
# Fire spreads faster in lower floors and near exits — higher risk in those areas
...
# AHP: Fire — Risk more important than distance (4:1)
fire_matrix = np.array([[1, 4], [1/4, 1]])
fire_weights = ahp_weights(fire_matrix)

print("\n🔥 FIRE Scenario AHP Weights (Risk, Distance):", fire_weights)
...
plt.plot(x, np.mean(fire_best_solution, axis=1)[:num_rooms],
         label='🔥 Fire Evacuation Route', color='red', linewidth=2)
plt.plot(x, np.mean(eq_best_solution, axis=1)[:num_rooms],
         label='🌎 Earthquake Evacuation Route', color='blue', linewidth=2, linestyle='--')
...
print("✅ Results saved as Fire_Evacuation_Route.csv and Earthquake_Evacuation_Route.csv")